...
def RunAmplifiersFeedback(program, inputs):
  ...
  while not done:
    for i in range(5): # This is easier that A-E.
      done = ExecuteInstructions(amps[i].program, amps[i].input_queue, amps[i%5].output_queue)
  return amps[4].output_queue.get()
...
def main():
  ...
  for inp in itertools.permutations(range(5)):
    output = RunAmplifiers(content, inp)
    if output > max_output:
      max_output = output
      max_setting = inp
  
  print 'Max Output: ', max_output, ' with Setting: ', max_setting
